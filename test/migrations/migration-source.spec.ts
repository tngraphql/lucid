...
test.group('MigrationSource', (group) => {
  ...
  test('get list of migration files from database/migrations.js', async (assert) => {
    ...
    const migrationSource = new MigrationSource(db.getRawConnection('primary')!.config, app as any)
    ...
    assert.deepEqual(directories.map((file) => {
      return { absPath: file.absPath, name: file.name.replace(/\\/g, '/') }
    }), [
      {
  ...
  test('sort multiple migration directories seperately', async (assert) => {
    ...
    const migrationSource = new MigrationSource(config, app as any)
