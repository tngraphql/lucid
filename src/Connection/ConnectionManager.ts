...
import { ConnectionConfig, ConnectionNode, ReportNode } from '../Contracts/Connection/types';
...
export class ConnectionManager implements ConnectionManagerContract {
    ...
    public async closeAll(release: boolean = false): Promise<void> {
        await Promise.all(Array.from<string>(this.connections.keys()).map((name) => this.close(name, release)));
    }
    ...
    public async report(): Promise<HealthReportEntry & { meta: ReportNode[] }> {
        const reports = await Promise.all(
            Array.from<string>(this.connections.keys())
